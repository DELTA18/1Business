    ...
    <main className="flex flex-col items-center justify-center gap-10 p-24 bg-gray-950 min-h-screen">
      ...
      {session ? (
        <>
          ...
          <p className=" font-poppins text-xl font-semibold text-green-400 mb-4">Welcome, {session.user?.name}</p>

          <CreatePost />

          <section className="mt-12 w-full max-w-5xl">
            <h2 className="text-3xl font-bold text-white mb-6">Latest Busineess Ideas</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {posts.map((post: any) => (
  <div
    key={post._id}
    className="relative bg-white/10 border border-white/20 rounded-xl shadow-lg hover:shadow-xl transition-all p-6 backdrop-blur-md"
  >
        {/* Top-right updatedAt date */}
        <span className="absolute top-3 right-4 backdrop-blur-3xl font-bold text-xs text-gray-800 bg-white/40 px-2 py-1 rounded-full">
